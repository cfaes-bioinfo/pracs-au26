...
- **Go back** to earlier versions (e.g., after accidentally overwriting a file,
...
And: of the files you would *not* commit, which one most needs to be backed up some other way?
...
**c)** is _not_ derived: raw data can't be regenerated, but it's too large for Git.
So that's the file that most needs a backup elsewhere ---
not being in Git doesn't mean a file is unimportant!
:::
------>
...
- But you can switch to GUI usage later:
  ...
  - We recommend using the VS Code integration, and will briefly show this.
