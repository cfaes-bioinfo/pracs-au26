...
we'll put them into practice [right after](./w5b_git.qmd).
:::
...
Some "versioning"- and backup-related challenges for your **research project files**
you may run into:

::: {.columns}
::: {.column width="47%"}

### [Managing backups]{style="display:block; text-align:center;"}

How do you save periodic copies?

- *Do you only save versions of individual files?*\
  Space-efficient, but loses point-in-time project synchronization.
...
### [Managing changes]{style="display:block; text-align:center;"}
...
However, with a formal Version Control System like Git:
...
- You can go back to past states of your entire project, not just individual files.
- You can better see your history of changes.
...
- Usage is also possible on remote servers like OSC.
- It's straightforward to publicly (or privately) share your code.
...
In this course, you will learn the _basics_ of using version control.

TODO - expand? Mention AI?
...
Also within project folders, systematic file organization enables you to use
Git effectively.
:::
...
They are generated and used automatically by software ---
...
If you have small datasets, it may be appealing to add them,
e.g. because this makes for an easy way to share this with others.

That can be OK even if not exactly what Git is designed for --
...
a) Git automatically saves a new snapshot every time you save a file.
b) Git can show line-by-line changes for Excel files.
c) A Git repository's database is stored in a hidden `.git` directory.
...
::: callout-tip
#### Answer
a) **False**: you make commits manually, and that's by design.
b) **False**: Git can store binary files like Excel files,
   but it can't show line-by-line changes for them.
c) **True**: and don't mess with it!
:::
------>
...
- "**Staging**" files: mark changed and new files as ready to be committed.
...
Another way of visualizing the adding and committing of changes in Git:
...
You just created a new script `trim.sh` in your project directory.
Which single Git command will make Git start tracking the file
_and_ stage it for the next commit?
...
::: callout-tip
#### Answer
**c)**: `git add` does both --- it starts tracking a new file, and stages it
(new or changed) for the next commit.
:::
------>
