...
- Open the `week05/evo` dir you created last class.
  Since you've opened this folder this before, the easiest way to do so
...
- Open a new terminal and check that you are in the `evo` dir:
...
![](/img/vscode-recents.png){fig-align="center" width="90%" fig-alt="A screenshot of the VS Code 'Welcome' document, showing the 'Recent' section." .lightbox}

::: legend2
The `Welcome` document in VS Code, with its "**Recent**" section:
...
here, we'll use the most common choice, **GitHub**.
...
2. You'll be asked three questions, and for all three,
   ...
   For the passphrase questions, this means that your key has no passphrase,
   so you won't have to type one each time you push.
   The flip side is that anyone who gets into your OSC account could use the key.]:
...
- **Adding files in this interface**\
  ...
  (fixing that requires `git pull`, which we don't cover in class).
  The same happens if you accidentally let GitHub add a README when creating the repo.
  In that case, the easiest fix is to delete the new GitHub repo
...
2. **Upload ("push")** your local repo to remote using `git push`.
   ...
   #### Do you get asked for a user name and password? _(Click to expand)_
   If you accidentally used the HTTPS URL, `git push` will ask for your GitHub
   user name and password, and then fail even if you enter them correctly.

   You can fix this by running `git remote set-url` to change the URL
   but change the URL with `git remote set-url`
   ([see the box "Check or change the remote connection" below](#check-remote)).
   :::

3. Check the output, which should look similar to this:
...
- From the **currently active branch** in your local repo
  (for us, that's `main` --- our only branch)^[
...
::: {#check-remote .callout-note collapse="true"}
...
In the gray bar towards the top, GitHub shows the number of commits made
...
- If you'd like your avatar to be a custom profile picture instead of the
  default GitHub icon,
...
On the right-hand side, a <kbd>< ></kbd> button next to each commit allows you to
see the **state of the entire repo** (all files in the project) at the time of that commit:

![](/img/github-browse-repo-at-commit.png){fig-align="center" width="31%" fig-alt="A screenshot of the Browse Repository button on GitHub." .lightbox}

On the commit overview page, scroll down all the way and
**click the <kbd>< ></kbd> for the first commit** ---
...
(in public repos, anyone with a GitHub account can open an issue to report a problem or ask a question).
...
In **next week's exercises**, you'll practice tagging the instructors in an issue,
...
(unless you work on the repo from multiple computers --- then, you'd also need `git pull`):
...
- Open the file in the VS Code editor and copy the following text into it ---
  we'll include quite a bit of Markdown formatting,
  so we can later see how GitHub automatically renders Markdown files:

  ```{.file filename="README.md" .numberLines}
...
::: exercise
#### {{< fa user-edit >}} Self-study exercise: Commit and push again
...
2.  Predict which message you will see if you check the status of your repo.
    Then, see if your prediction was correct by running `git status`.
...
10. Do one-time Git setup (name, email, default branch name) with `git config`.
