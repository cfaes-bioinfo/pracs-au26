...
2. You should already have a `week15` folder from Monday's lecture
3. Create a new Quarto document by clicking `File` \> `New File` \>
   `Quarto Document` and save it inside the `week15` directory.
...
count_table_file <- "/fs/ess/PAS3493/share/week15/salmon.merged.gene_counts_length_scaled.tsv"
metadata_file <- "../garrigos/meta/metadata.tsv"
```

```{r, echo=FALSE}
count_table_file <- here::here(
  "assets/salmon.merged.gene_counts_length_scaled.tsv"
)
metadata_file <- here::here("assets/metadata.tsv")
```
...
meta <- meta_raw |>
  ...
  mutate(
    time = factor(time, levels = c("24hpi", "10dpi")),
    treatment = factor(
      treatment,
      levels = c("control", "cathemerium", "relictum")
    )
  )
```
...
DESeq2 expects counts as whole numbers (integers), and gene IDs as row names:
...
What are the summed and mean counts per gene, and the summed counts per sample?
We'll show the first six results for each:
...
- How does our plot compare to the PCA plot in the paper (Figure 1),
  in terms of the conclusions you just drew?
...
it will be relatively straightforward to customize the plot using your knowledge
of ggplot2.
...
Due to the log-transformation, the LFC increases more slowly than a raw fold-change:

- An LFC of **`1`** indicates a 2-fold difference
- An LFC of **`2`** indicates a 4-fold difference
- An LFC of **`3`** indicates an 8-fold difference
...
<details><summary>*Click for the solution for how to label it in the plot*</summary>
<hr style="height:1pt; visibility:hidden;" />
...
collapsed box "`NA` values in the DESeq2 results table" above
...
Wow! It looks like in every single time + treatment combination, all but one (or in one
case, two) of the samples have zero expression, but there are several extreme outliers.

For our focal comparison at `24hpi` (left panel/facet), comparing `control` vs `relictum`,
it looks like the difference between these two groups is solely due to the one outlier
...
So, we have to be careful with taking our statistical results at face value, and need to
...
write_tsv(as.data.frame(res_rc24), "results/DE/res_rc24.tsv")

# You can also save the whole DESeq2 object, so you can reload it later with readRDS()
saveRDS(dds, "results/DE/dds.rds")
```
...
- Run a Differential Expression (DE) analysis with DESeq2
...
- Another very common type of plot used for gene expression data (and omics data
  ...
  You'll create heatmaps in [the next lecture](./w15c_heatmaps.qmd).
