...
| proteomics      | proteins          | What is actively working     | [**mass spectrometry**]{.blue} |
| metabolomics    | metabolites       | What the net outcome is      | [**mass spectrometry**]{.blue} |
...
<!-- Spacing is to scale: dot-to-dot distance = 0.42rem per year elapsed.
     Since each event is one line (~1.2rem) tall, and the year + dot are
     centered on the event's *padding* box, the gaps use margin-bottom
     (outside the box) rather than --tl-gap (padding, which would drag
     the year and dot down). Per event: margin = years * 0.42rem - 1.2rem -->
...
::: {.event data-label="1986" .fragment style="margin-bottom: 7.2rem;"}
First automated **Sanger** sequencer
:::

::: {.event data-label="2006" .fragment style="margin-bottom: 0.9rem;"}
First high-throughput sequencer,<br> by **Illumina**
:::

::: {.event data-label="2011" .fragment style="margin-bottom: 0.48rem;"}
First long-read sequencer, by **PacBio**
:::

::: {.event data-label="2015" .fragment}
...
<details class="details-quiz">
<summary>For which of these applications are longer reads especially useful?</summary>
<!-----
- Genome assembly
...
- _Taxonomic classification of reads against a reference database._
...
simply generated at random from input such as genomic DNA:
...
<details class="details-quiz">
<summary>Shorter than the combined read length (but longer than a single read), which leads to?</summary>
...
<details class="details-quiz">
<summary>Shorter than the *single* read length, which leads to?</summary>
...
- An overview of the **RNA-Seq data analysis steps** we'll perform on the @garrigós2025 dataset.
