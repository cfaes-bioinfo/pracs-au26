...
Today (and during part of Wednesday's meeting),
...
In [week 10](../week10/w10a_workflow.qmd),
...
Recall from week 10 that the ["nf-core" initiative](https://nf-co.re/) (@ewels2020)
...
```bash-out-solo
Pulling Singularity image https://community-cr-prod.seqera.io/docker/registry/v2/blobs/sha256/52/52ccce28d2ab928ab862e25aae26314d69c8e38bd41ca9431c67ef05221348aa/data [cache /fs/ess/PAS3493/people/jelmer/week11/testrun/work/singularity/community-cr-prod.seqera.io-docker-registry-v2-blobs-sha256-52-52ccce28d2ab928ab862e25aae26314d69c8e38bd41ca9431c67ef05221348aa-data.img]
Pulling Singularity image https://depot.galaxyproject.org/singularity/fq:0.12.0--h9ee0642_0 [cache /fs/ess/PAS3493/people/jelmer/week11/testrun/work/singularity/depot.galaxyproject.org-singularity-fq-0.12.0--h9ee0642_0.img]
Pulling Singularity image https://community-cr-prod.seqera.io/docker/registry/v2/blobs/sha256/9b/9becad054093ad4083a961d12733f2a742e11728fe9aa815d678b882b3ede520/data [cache /fs/ess/PAS3493/people/jelmer/week11/testrun/work/singularity/community-cr-prod.seqera.io-docker-registry-v2-blobs-sha256-9b-9becad054093ad4083a961d12733f2a742e11728fe9aa815d678b882b3ede520-data.img]
Pulling Singularity image https://depot.galaxyproject.org/singularity/fastqc:0.12.1--hdfd78af_0 [cache /fs/ess/PAS3493/people/jelmer/week11/testrun/work/singularity/depot.galaxyproject.org-singularity-fastqc-0.12.1--hdfd78af_0.img]
Pulling Singularity image https://depot.galaxyproject.org/singularity/python:3.9--1 [cache /fs/ess/PAS3493/people/jelmer/week11/testrun/work/singularity/depot.galaxyproject.org-singularity-python-3.9--1.img]
```
...
The `work` dir is in this case in its default location, which is your current
...
As mentioned above, the hexadecimals between square brackets (e.g. `[fb/68d030]`)
...
```bash
# Go back to /fs/ess/PAS3493/people/$USER/week11
cd ..
...
and specify the so-called "strandedness" of your RNA-Seq library.
...
In turn, stranded libraries can be prepared either in
...
the R1 and R2 FASTQ file suffixes, the input FASTQ dir (`../garrigos/fastq`),
...
```bash-out
sample,fastq_1,fastq_2,strandedness
S63,../garrigos/fastq/S63_R1.fastq.gz,../garrigos/fastq/S63_R2.fastq.gz,forward
S64,../garrigos/fastq/S64_R1.fastq.gz,../garrigos/fastq/S64_R2.fastq.gz,forward
S65,../garrigos/fastq/S65_R1.fastq.gz,../garrigos/fastq/S65_R2.fastq.gz,forward
S66,../garrigos/fastq/S66_R1.fastq.gz,../garrigos/fastq/S66_R2.fastq.gz,forward
```
...
- `paste - -` will paste those FASTQ files side-by-side in two columns ---
...
we want to have the pipeline submit Slurm batch jobs so it will efficiently
...
- Since the pipeline has many good default settings for (Slurm) batch jobs,
  ...
  process.clusterOptions='--account=PAS3493'
  ```
...
we'll keep the shell script flexible by having it accept arguments that represent
...
(see the [pipeline's documentation](https://nf-co.re/rnaseq/3.22.0/docs/usage)),
...
::: exercise
#### {{< fa user-edit >}} Exercise: Find the right pipeline options
Take a look at the
...
2. Say you want to (a) remove rRNA, (b) skip the STAR alignment and quantify
   with Salmon only, and (c) have Nextflow produce an execution timeline report.
   For each of these three, decide **before looking it up** whether you expect
   a single-dash or a double-dash option, then find the actual option name(s).
   Was your prediction right in all three cases?
   Why does the option for (c) have only one dash?

<details><summary> _Click for the solution_ </summary>

- (a) Removing rRNA is a pipeline-specific option (double dash), since it's
  specific to this pipeline: **`--remove_ribo_rna`**.

- (b) Skipping STAR and quantifying with Salmon only is also pipeline-specific
  (double dash), and takes two options:
  **`--skip_alignment`** (to skip STAR) and **`--pseudo_aligner salmon`**
  (to run Salmon on its own).

- (c) Producing an execution timeline report is a **general Nextflow option**
  (single dash): **`-with-timeline`**.
  Unlike (a) and (b), this isn't something specific to the _rnaseq_ pipeline --
  every Nextflow pipeline can produce this kind of report,
  because it's a feature of Nextflow itself rather than of this particular pipeline.
...
Now, we're ready to create the full shell script.
...
- `#SBATCH` options: note that these are *only* for the "main" Nextflow process,
  ...
  via the `nextflow.config` file if needed, but we won't do that here.
...
export NXF_SINGULARITY_CACHEDIR=/fs/scratch/PAS3493/containers
```
:::
...
You will add the code to do so to the `README.md` file we created above.

### Define variables to pass to the shell script
...
outdir=results/nfc-rnaseq
workdir=/fs/scratch/PAS3493/$USER/week11/nfc-rnaseq-work

# Defining the pipeline inputs
samplesheet=metadata/nfc-samplesheet.csv
fasta=../garrigos/ref/GCF_016801865.2_TS_CPP_V2_genomic.fna
gtf=../garrigos/ref/annot.gtf
```
...
```
```bash-out
-rw-rw----+ 1 jelmer PAS0471 123M Oct 23 12:27 ../garrigos/ref/annot.gtf
-rw-rw----+ 1 jelmer PAS0471 547M Dec 22  2022 ../garrigos/ref/GCF_016801865.2_TS_CPP_V2_genomic.fna
...
```bash-out
 ...
 42175824       cpu                 nf-NFCORE_RNASEQ_RNASEQ_FASTQ_QC_TRIM_FILTER_SETSTRANDEDNESS_SORTMERNA_(S79)  RUNNING       0:53   16:00:00   17 p0111 2026-11-30T08:49
 42175825       cpu                 nf-NFCORE_RNASEQ_RNASEQ_FASTQ_QC_TRIM_FILTER_SETSTRANDEDNESS_SORTMERNA_(S80)  RUNNING       0:53   16:00:00   17 p0116 2026-11-30T08:49
 42175826       cpu                 nf-NFCORE_RNASEQ_RNASEQ_FASTQ_QC_TRIM_FILTER_SETSTRANDEDNESS_SORTMERNA_(S83)  RUNNING       0:53   16:00:00   17 p0121 2026-11-30T08:49
 42175827       cpu                 nf-NFCORE_RNASEQ_RNASEQ_FASTQ_QC_TRIM_FILTER_SETSTRANDEDNESS_SORTMERNA_(S85)  RUNNING       0:53   16:00:00   17 p0122 2026-11-30T08:49
 42175780       cpu          nf-NFCORE_RNASEQ_PREPARE_GENOME_STAR_GENOMEGENERATE_(GCF_016801865.2_TS_CPP_V2_genomic.fna)  RUNNING       3:19   16:00:00   17 p0116 2026-11-30T08:46
 42175708       cpu                                                                                        nfc-rnaseq.sh  RUNNING       5:19   12:00:00    1 p0222 2026-11-30T08:44
```
:::
...
less -R slurm-nfc-rnaseq*
```
```bash-out
Starting script nfc-rnaseq.sh
Sun Nov 30 08:45:14 AM EST 2025
Samplesheet:          metadata/nfc-samplesheet.csv
Reference FASTA:      ../garrigos/ref/GCF_016801865.2_TS_CPP_V2_genomic.fna
Reference GTF:        ../garrigos/ref/annot.gtf
Output dir:           results/nfc-rnaseq
Nextflow work dir:    /fs/scratch/PAS3493/jelmer/week11/nfc-rnaseq-work
...
```bash-out-solo
[62/846bb5] Submitted process > NFCORE_RNASEQ:RNASEQ:FASTQ_QC_TRIM_FILTER_SETSTRANDEDNESS:FQ_LINT (S63)
[a1/e6ec8f] Submitted process > NFCORE_RNASEQ:RNASEQ:FASTQ_QC_TRIM_FILTER_SETSTRANDEDNESS:FQ_LINT (S64)
...
[23/92e452] Submitted process > NFCORE_RNASEQ:RNASEQ:FASTQ_QC_TRIM_FILTER_SETSTRANDEDNESS:FQ_LINT (S65)
[ca/1cbfe9] Submitted process > NFCORE_RNASEQ:RNASEQ:FASTQ_QC_TRIM_FILTER_SETSTRANDEDNESS:FQ_LINT (S66)
[34/054d8c] Submitted process > NFCORE_RNASEQ:RNASEQ:FASTQ_QC_TRIM_FILTER_SETSTRANDEDNESS:FQ_LINT (S67)
[32/08014c] Submitted process > NFCORE_RNASEQ:RNASEQ:FASTQ_QC_TRIM_FILTER_SETSTRANDEDNESS:FQ_LINT (S68)
[4e/fe7605] Submitted process > NFCORE_RNASEQ:RNASEQ:FASTQ_QC_TRIM_FILTER_SETSTRANDEDNESS:FQ_LINT (S70)
[46/448512] Submitted process > NFCORE_RNASEQ:RNASEQ:FASTQ_QC_TRIM_FILTER_SETSTRANDEDNESS:FQ_LINT (S71)
```
...
```bash-out-solo
[ca/b7b202] Submitted process > NFCORE_RNASEQ:RNASEQ:BEDGRAPH_BEDCLIP_BEDGRAPHTOBIGWIG_REVERSE:UCSC_BEDGRAPHTOBIGWIG (S79)
[86/67dd0a] Submitted process > NFCORE_RNASEQ:RNASEQ:BEDGRAPH_BEDCLIP_BEDGRAPHTOBIGWIG_FORWARD:UCSC_BEDGRAPHTOBIGWIG (S64)
-[nf-core/rnaseq] Pipeline completed successfully -
...
First, move into the `work` dir for the `SALMON_QUANT` job for sample `S67`.

<details><summary> _Click for the solution_ </summary>

- Go through the lines for `SALMON_QUANT` jobs in the pipeline's Slurm log file
  to find the one with sample `S67`.
  You can do so by searching, e.g. with <kbd>Ctrl+F</kbd> in the editor,
  or with `grep` in the terminal:

  ```bash
  grep "SALMON_QUANT" slurm-nfc-rnaseq-*.out | grep "S67"
  ```
  ```bash-out-solo
  [ba/268636] Submitted process > NFCORE_RNASEQ:RNASEQ:QUANTIFY_STAR_SALMON:SALMON_QUANT (S67)
  ```

- From this line, we see that the job was run in `ba/268636*` within our `work` dir.
  Our base `work` dir is `/fs/scratch/PAS3493/$USER/week11/nfc-rnaseq-work`,
  so the full path to the job's `work` dir can be found with:

  ```bash
  # (Note the trailing asterisk to match the full dir name)
  cd /fs/scratch/PAS3493/$USER/week11/nfc-rnaseq-work/ba/268636*
  ```
</details>
...
- List the files in this dir --
  ...
  drwxr-xr-x 5 jelmer PAS0471 4.0K Nov 30 08:53 S67
  lrwxrwxrwx 1 jelmer PAS0471  127 Nov 30 08:53 S67.Aligned.toTranscriptome.out.bam -> /fs/scratch/PAS3493/jelmer/week11/nfc-rnaseq-work/b9/40ec61ed2b7b9bc2a7d4ee4fb15948/S67.Aligned.toTranscriptome.out.bam
  -rw-r--r-- 1 jelmer PAS0471 1.3K Nov 30 08:53 S67_meta_info.json
  lrwxrwxrwx 1 jelmer PAS0471  112 Nov 30 08:53 GCF_016801865.2.filtered.gtf -> /fs/scratch/PAS3493/jelmer/week11/nfc-rnaseq-work/09/0a23314e90aa3ad81d29341c86e333/GCF_016801865.2.filtered.gtf
  lrwxrwxrwx 1 jelmer PAS0471  105 Nov 30 08:53 genome.transcripts.fa -> /fs/scratch/PAS3493/jelmer/week11/nfc-rnaseq-work/ce/bca98f4ea57de4e83c87d76db88b3c/genome.transcripts.fa
...
- Open and skim through the `.command.log` file:
  ...
  # [ alignments ] => { S67.Aligned.toTranscriptome.out.bam }
  # [ output ] => { S67 }
  Logs will be written to S67/logs
...
**[copy of the MultiQC report on this website](../assets/multiqc_report_nfcore.html)**).
...
The **RSeQC** > **Infer experiment** (library strandedness) plot.
If your library is:
...
![This library is clearly forward-stranded, as we indicated in our sample sheet.](../img/multiqc_strandedness.png){fig-align="center" width="85%"}
