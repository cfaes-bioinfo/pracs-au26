...
In this course, we have covered the following practices that benefit reproducibility:
...
- Date: First created on 2026-04-01, last modified on 2026-08-25
...
- OSC working dir: /fs/ess/PAS3493/people/jelmer
...
- **Input files**:
  ...
  - Input file paths are set below and can be adjusted to your situation.
    - If you're running this at OSC and have access to project PAS3493,
...
## Step 4: Trimmed read alignment to a reference genome with STAR
...
The `salmon.sh` script runs for one sample at a time and takes 3 arguments -
the input BAM file produced by `star.sh`, a reference GTF file,
and the desired output dir:
...
it is important to realize that you **cannot** run all the code at once.

<details><summary>Why can't all the code be run at once? _(Click to see the solution)_</summary>
This is because most steps need files produced by an earlier step as their input.
...
a job to actually start once one or more other jobs have (successfully) finished
running.
...
- **Automation**
  ...
  - Integration with software environments (Conda, containers).
...
- Accept that your workflow isn't automated, and use Markdown protocols like the one above
...
It currently has over 150 complete pipelines --
...
which we'll run in week 11 of the course:
