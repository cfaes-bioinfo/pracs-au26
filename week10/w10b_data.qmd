...
pagetitle: "w10b: data management"
date: 2026-10-28
...
<!---
Maybe comment on Excel files -- not always a bad thing
---->
...
and they may have been labor-intensive and expensive to produce.
...
Try to download some files from `/fs/ess/PAS3493/people/$USER` to your local computer,
and vice versa.
:::
...
We actually do need to do this because my `garrigos` repo contains only
...
2. Type `wget` and <kbd>Space</kbd> in your terminal, paste the URL,
   and execute the command:
    ...
    2026-10-23 10:19:12 (23.0 MB/s) - ‘GCF_016801865.2_TS_CPP_V2_genomic.fna.gz’ saved [172904433/172904433]
    ```
...
4. Move it to your `garrigos` dir:

   ```bash
   mv -v GCF_016801865.2_TS_CPP_V2_genomic.fna.gz garrigos/ref
   ```

5. Create a new README file...

   ```bash
   touch garrigos/ref/README.md
   ```

   ...and in it, add a note about how you obtained the file:

   ````markdown
   Downloaded reference genome FASTA from NCBI on 2026-10-23 with:
...
- For downloads from the Sequence Read Archive (SRA),
  ...
  and [fastq-dl](https://github.com/rpetit3/fastq-dl).
...
- **Owner (or "user")** (`u`) ---
  By default, this is the person that created the file or dir.
...
its "**group**" will include all members of the OSC project PAS3493.
That is true even if the primary group displayed in the `ls -l` output shown above
...
```bash-out
total 963552
-rw-rw----+ 1 jelmer PAS0471 21266097 Sep  9 13:46 S63_R1.fastq.gz
-rw-rw----+ 1 jelmer PAS0471 22700521 Sep  9 13:46 S63_R2.fastq.gz
```
...
```bash-out
total 963552
-r--r--r--+ 1 jelmer PAS0471 21266097 Sep  9 13:46 S63_R1.fastq.gz
-r--r--r--+ 1 jelmer PAS0471 22700521 Sep  9 13:46 S63_R2.fastq.gz
```
...
chmod a=r sandbox/S63_R1.fastq.gz
...
You'll be prompted for every file! **Answer `n` (no) to keep the file for now.**
If you answer `y` (yes), the file is in fact removed --
file permissions like this only protect against *accidentally* overwriting or
deleting a file, they don't prevent the file's owner (or a system administrator)
from removing it on purpose.
