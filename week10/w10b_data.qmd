...
- To set read + write + execute (`rwx`) permissions for all (`a`):
  ...
  ```
  ```bash-out
  -rwxrwxrwx+ 1 jelmer PAS3493 0 Mar  7 13:36 testfile.txt
  ```
...
- To remove write (`w`) permissions for others (`o`):
  ...
  ```
  ```bash-out
  -rwxrwxr-x+ 1 jelmer PAS3493 0 Mar  7 13:36 testfile.txt
  ```
...
```
```bash-out
rm: remove write-protected regular empty file ‘garrigos/fastq/S63_R1.fastq.gz’?
```
