---
title: "Week 10 exercises: A longer workflow"
pagetitle: "Week 10 exercises"
date: 2026-10-26
...
- We've already made the shell scripts to run individual steps.
  Copy them into your `scripts` dir:

  ```bash
  cp -v /fs/ess/PAS3493/share/scripts/w10_ex/* scripts/
  ```
  ```bash-out
  '/fs/ess/PAS3493/share/scripts/w10_ex/fastqc.sh' -> 'scripts/fastqc.sh'
  '/fs/ess/PAS3493/share/scripts/w10_ex/multiqc.sh' -> 'scripts/multiqc.sh'
  '/fs/ess/PAS3493/share/scripts/w10_ex/star_align.sh' -> 'scripts/star_align.sh'
  '/fs/ess/PAS3493/share/scripts/w10_ex/star_index.sh' -> 'scripts/star_index.sh'
  '/fs/ess/PAS3493/share/scripts/w10_ex/trimgalore.sh' -> 'scripts/trimgalore.sh'
  ```
...
- Copy the following into your `README.md`:
  ...
  2. Download the reference genome assembly FASTA from NCBI as follows:
...
The input files will be the `garrigos` ones --
...
[we downloaded earlier](./w10b_data.html#wget).
...
[the one laid out in this week's first lecture](./w10a_workflow.html#structuring-your-analysis-workflow),
but not identical!
...
- OSC working dir: /fs/ess/PAS3493/people/jelmer/week10/exercises
...
```bash
# Inputs:
fastq_dir=garrigos/fastq
ref_assembly=garrigos/ref/GCF_016801865.2_TS_CPP_V2_genomic.fna
ref_annotation=garrigos/ref/annot.gtf
...
- Monitor the jobs:
  ...
  2. `less slurm-fastqc*` to read over at least one Slurm log file
...
- Expected/example Slurm log file contents:
  ...
  /// MultiQC 🔍 v1.31
         ...
         file_search | Search path: /fs/ess/PAS3493/people/jelmer/week10/exercises/results/fastqc
...
[Link to my MultiQC report file](../assets/multiqc_report_week10.html)
...
- You've seen the TrimGalore logs and output files before.
  ...
  ```bash-out-solo
  S86_R1.fastq.gz   S86_R2.fastq.gz
  ```
...
- Expected Slurm log file contents:
  ...
  # Input assembly FASTA file:      garrigos/ref/GCF_016801865.2_TS_CPP_V2_genomic.fna
  # Input annotation GTF file:      garrigos/ref/annot.gtf
  # Output dir:                     results/star/index

  INFO:    Using cached SIF image
  INFO:    gocryptfs not found, will not be able to use gocryptfs
  	/opt/conda/bin/STAR-avx2 --runMode genomeGenerate --genomeFastaFiles garrigos/ref/GCF_016801865.2_TS_CPP_V2_genomic.fna --sjdbGTFfile garrigos/ref/annot.gtf --genomeDir results/star/index --runThreadN 16
...
<details><summary>**10.** Add code to your README to run `star_align.sh`, and run it _(click to expand)_</summary>
...
- Expected Slurm log file contents (one file):
  ...
  # Input R1 FASTQ file:            results/trimgalore/S63_R1.fastq.gz
  # Input R2 FASTQ file:            results/trimgalore/S63_R2.fastq.gz
  # Input genome index:             results/star/index
  # Input annotation GTF:           garrigos/ref/annot.gtf
  # Output dir:                     results/star

  INFO:    Using cached SIF image
  INFO:    gocryptfs not found, will not be able to use gocryptfs
  	/opt/conda/bin/STAR-avx2 --readFilesIn results/trimgalore/S63_R1.fastq.gz results/trimgalore/S63_R2.fastq.gz --genomeDir results/star/index --sjdbGTFfile garrigos/ref/annot.gtf --runThreadN 8 --readFilesCommand zcat --outFileNamePrefix results/star/S63_ --outSAMtype BAM SortedByCoordinate
  	STAR version: 2.7.11b   compiled: 2024-07-03T14:39:20+0000 :/opt/conda/conda-bld/star_1720017372352/work/source
...
- Expected output files:
  ...
  ```bash-out
  total 21M
  -rw-rw----+ 1 jelmer PAS0471 1.4M Oct 23 12:13 S63_Aligned.sortedByCoord.out.bam
  -rw-rw----+ 1 jelmer PAS0471 2.0K Oct 23 12:13 S63_Log.final.out
  -rw-rw----+ 1 jelmer PAS0471  18K Oct 23 12:13 S63_Log.out
  -rw-rw----+ 1 jelmer PAS0471  246 Oct 23 12:13 S63_Log.progress.out
  -rw-rw----+ 1 jelmer PAS0471 3.5K Oct 23 12:13 S63_SJ.out.tab
  drwx------+ 2 jelmer PAS0471 4.0K Oct 23 12:13 S63__STARgenome
  -rw-rw----+ 1 jelmer PAS0471 1.8M Oct 23 12:13 S64_Aligned.sortedByCoord.out.bam
  -rw-rw----+ 1 jelmer PAS0471 2.0K Oct 23 12:13 S64_Log.final.out
  -rw-rw----+ 1 jelmer PAS0471  18K Oct 23 12:13 S64_Log.out
  -rw-rw----+ 1 jelmer PAS0471  246 Oct 23 12:13 S64_Log.progress.out
  -rw-rw----+ 1 jelmer PAS0471 5.4K Oct 23 12:13 S64_SJ.out.tab
  drwx------+ 2 jelmer PAS0471 4.0K Oct 23 12:13 S64__STARgenome
  -rw-rw----+ 1 jelmer PAS0471 1.2M Oct 23 12:13 S65_Aligned.sortedByCoord.out.bam
  # [...output truncated...]
  ```
