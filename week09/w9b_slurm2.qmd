...
2. In the terminal, navigate to your `week09` dir
...
- passed on the command line: `sbatch --account=PAS3493 myscript.sh`
  _(has precedence over the next)_ and/or
- added at the top of the script you're submitting: `#SBATCH --account=PAS3493`.
...
Note that your job will be **killed** (stopped) as soon as it hits the time limit!
...
:::callout-tip
##### The time you reserve is the "**wall time**"
...
```bash-out-solo
slurmstepd: error: *** JOB 23641567 ON p0133 CANCELLED AT 2026-04-04T14:55:24 DUE TO TIME LIMIT ***
```
</details>
-->
...
Standard cores have 4 GB of memory "on them", and therefore,
the default amount of memory you will get is 4 GB per reserved core.
...
To request that a batch job runs on a non-default node type, you should use
...
The output that programs (including basic shell commands) print to screen can
be divided into two types:

1. **Standard output** (`stdout`) for regular output
2. **Standard error** (`stderr`) for error messages and sometimes warning messages

Most of the time, it's not obvious that these are two separate types,
but it is possible to treat them separately. For example,
...
| Only allow job to begin after a specific time | `--begin=2026-04-05T12:00:00`
...
fastq_file=../garrigos/fastq/S63_R1.fastq.gz
ls -lh "$fastq_file"
```
```bash-out
...
```bash-out
total 1.1M
-rw-rw----+ 1 jelmer PAS0471 718K Sep 30 09:21 S63_R1_fastqc.html
-rw-rw----+ 1 jelmer PAS0471 364K Sep 30 09:21 S63_R1_fastqc.zip
```
...
if you preface the command with `echo`,
...
2. Check the last few lines of all of them:
   ...
   Approx 85% complete for S64_R1.fastq.gz
   Approx 90% complete for S64_R1.fastq.gz
   Approx 95% complete for S64_R1.fastq.gz
   Approx 100% complete for S64_R1.fastq.gz
   Analysis complete for S64_R1.fastq.gz
   # Used FastQC version:
   FastQC v0.12.1
...
3. Check for the presence of the FastQC output files:
   ...
   ```bash-out
   total 25M
   -rw-rw----+ 1 jelmer PAS0471 718K Sep 30 06:35 S63_R1_fastqc.html
   -rw-rw----+ 1 jelmer PAS0471 364K Sep 30 06:35 S63_R1_fastqc.zip
   -rw-rw----+ 1 jelmer PAS0471 714K Sep 30 06:36 S64_R1_fastqc.html
   -rw-rw----+ 1 jelmer PAS0471 366K Sep 30 06:36 S64_R1_fastqc.zip
   -rw-rw----+ 1 jelmer PAS0471 713K Sep 30 06:36 S65_R1_fastqc.html
   -rw-rw----+ 1 jelmer PAS0471 367K Sep 30 06:36 S65_R1_fastqc.zip
   -rw-rw----+ 1 jelmer PAS0471 718K Sep 30 06:36 S66_R1_fastqc.html
   -rw-rw----+ 1 jelmer PAS0471 372K Sep 30 06:36 S66_R1_fastqc.zip
   -rw-rw----+ 1 jelmer PAS0471 710K Sep 30 06:36 S67_R1_fastqc.html
   # [...output truncated...]
   ```
...
In this case, it does make sense to **keep these files** around somewhere,
...
- When you've submitted many jobs that run the same script for different samples/files:
  ...
  - Check whether no jobs have failed: via email when using `--mail-type=FAIL`,
