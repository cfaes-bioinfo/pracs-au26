...
while [an optional self-study page covers Conda](../bonus/conda.qmd).
...
(without this extension, HTML files would only open as plain-text source documents).
...
for example, `module spider FastQC` would also find the `fastqc` module.
:::
...
Modules are convenient, but not all software is available as a module.
Containers are a more universal approach, with the added benefit of
improved reproducibility and portability^[
  By portable, we mean that something that works on one computer will also work
  on another computer.
] --- as you'll see below.
...
Let's try to load the most recent version, currently `4.5.2`:

```bash
module load R/4.5.2
```
```bash-out
Lmod has detected the following error:  These module(s) or extension(s) exist but cannot be loaded as requested: "R/4.5.2"
   You encountered this error for one of the following reasons:
   1. Missing version specification: On Pitzer, you must specify an available version.
   2. Missing required modules: Ensure you have loaded the appropriate compiler and MPI modules.
   Try: "module spider R/4.5.2" to view available versions or required modules.
   If you need further assistance, please contact oschelp@osc.edu with the subject line "lmod error: R/4.5.2"
```

Didn't work! As the error message suggests,
let's use `module spider` specifically on this version:
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------
  R: R/4.5.2
----------------------------------------------------------------------------------------------------------------------------------------------------------------
    You will need to load all module(s) on any one of the lines below before the "R/4.5.2" module is available to load.
      gcc/12.3.0
...
So, it appears you first need to load the module `gcc/12.3.0` before you can load
`R/4.5.2`.
...
(`gcc` and `intel` are two different _compilers_ ---
programs that translate code into a runnable program.
OSC has one of these two compilers loaded by default,
and different pieces of software are built with (and need) only one of them.
Since we explicitly loaded `gcc` above,
the module system swapped out the default `intel` compiler for it.)

Finally, let's check that R now indeed is available:
...
2. The command to run STAR is `STAR`, in uppercase.
   Try loading the module with an uppercase name too, i.e. `module load STAR/<version>`
   (with the version you found above). What happens, and why?
   Then, load the module correctly and test whether you can use the program
   by running the command `STAR` (in this case, without adding any options).

   <!----
   <details><summary> {{< fa check-circle >}} _Click for the solution._ </summary>

   This fails, because `module load` is case-sensitive and the module name
   is lowercase `star`, even though the command is uppercase `STAR`:

   ```bash
   module load STAR/2.7.10b
   ```
   ```bash-out
   Lmod has detected the following error:  The following module(s) are unknown: "STAR/2.7.10b"
   ```
...
3. Try running the command you wrote in the previous step.
   ...
   So, let's create the output dir first, and then rerun the command:
...
6. Explore the file. The "Summary" on the left gives each graph a
   green check (pass), orange exclamation mark (warning), or red cross (fail).
   Did any graphs get a warning or fail?
   Pick one, and try to figure out from the graph what FastQC is flagging.
...
::: {.callout-tip}
#### Don't take FastQC's flags at face value
FastQC's pass/warn/fail thresholds assume random genomic DNA sequences.
For other data types, some "failures" are expected: for example, RNA-seq data
typically gets a warning or fail for _Sequence Duplication Levels_,
since highly expressed genes produce many identical reads.
:::
...
Because it brings along its own dependencies,
a container works the same way on any computer that can run containers.
...
Recall that a container is like a small, separate computer system.
...
- **Reproducibility:** OSC periodically retires older modules as it updates its software,
  so a module you use today may no longer exist in a few years.
  A container image, on the other hand, doesn't change:
  the link you used will always give you the exact same program version(s).
...
2. A labmate wants to check the version of MultiQC (a program that summarizes FastQC results),
   and runs the command below. It fails --- why?
   ...
   you'll get one in Graded Assignment 3.
...
::: {.callout-note collapse="true"}
#### What if a program isn't available as a module or container? _(Click to expand)_

As mentioned above, nearly all bioinformatics programs are available through
Seqera Containers.
But if you do run into a program that isn't, you have two other options:

- **Contact OSC Help**\
  They may be able to help you install the program yourself, or,
  for commonly used software, may be willing to do a system-wide installation
  (i.e., make it available as a module).
  See the [previous lecture](w6a_osc.qmd) for how to contact them.
...
- **Install the software "manually"**\
  ...
  This can get very tricky at OSC,
  because you don't have administrator privileges
  and will often run into issues with dependencies
  (other software that the program depends on).
:::
