...
and it's is a good introductory example of a CLI tool.
...
![A FastQC per-base quality score graph for files with reasonably good quality reads.<br>The y-axis shows Phred quality scores (higher is better, see also the color-coding),<br>and the x-axis shows the position along the read.](../img/fastqc_good.png){fig-align="center" width="70%" fig-alt="An example FastQC per-base quality graph." .lightbox}

## Loading software at OSC using modules

As discussed in the previous lecture, much of the software installed at OSC can
only be used after its **module is explicitly loaded**.
You can load, unload, and search for available software modules using the
`module` command.

::: {.callout-warning appearance="simple"}
The three OSC clusters mostly have the same software installed,
...
Other module software functionality is also accessed using `module` commands.
...
- To test for the program's availability, we ran `fastqc -v`,
  which would print the FastQC version if it were available.
...
we will focus more closely on the more universal approach of using containers,
which has the added benefit of being more reproducible.
That said, to learn a bit more about modules, you can read the boxes below.
...
3. Use `module spider` to see if the program Salmon^[
   ...
   Why is it not available as a module?
...
Now that we've loaded the FastQC module, let's try to run it on a FASTQ file.
To analyze a FASTQ file with default FastQC settings,
the command is simply `fastqc` followed by the FASTQ file path ---
and we could use one of the FASTQ files in our @garrigós2025 practice data set:
...
consider that --- as you already know from standard shell commands ---
most bioinformatics tools also have a `-h` and/or `--help` option
that prints usage information to the screen.
Let's try that:
...
1. Look at FastQC's help info, and figure out which option can be used to specify
...
2. Adding that option, what would the full command be to run FastQC on the
   `S63_R1.fastq.gz` file, and write the output files to a `results/fastqc` dir?
