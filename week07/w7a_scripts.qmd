...
4. _Optional: Create a new Markdown file for class notes and save it in your `week07` dir_


## Intermezzo: VS Code revisited

#### TODO - Settings? Eg remove minimap
...
- When you **reference a non-existent ("unset") variable**,
  ...
  (you'll learn about variables below).
...
Now, let's use variables for the input file and the output dir in your FastQC script:
...
{{< fa user-edit >}} _Update your script with the code above,_
_and run it again to check that it still works._
...
# Run scripts with 2 arguments:
bash scripts/fastqc.sh data/sampleA.fastq.gz results/fastqc   # An input file and an output dir
bash scripts/printname.sh John Doe                             # Strings representing names
```
...
that have arguments, what are these variables and their values?

<details><summary>_Click here for the solution_</summary>
<hr style="height:1pt; visibility:hidden;" />

- In `bash scripts/fastqc.sh data/sampleA.fastq.gz results/fastqc`,
  two arguments are passed to the script:
  the first one (`data/sampleA.fastq.gz`) will be stored in `$1`,
  and the second one (`results/fastqc`) in `$2`.
...
In your FastQC script,
you can now simply replace the hard-coded file paths with `$1` and `$2`:
