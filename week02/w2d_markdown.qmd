...
- Has become even more prevalent with the rise of GenAI:
  chatbots write their responses in Markdown,
...
- Three `###` makes a level-3 header --- and so forth for smaller headers
...
::: {.callout-tip appearance="simple"}
Mind the order in a `[text](url)` link: the **`[`**link text**`]`** comes first,
and then the **`(`**URL**`)`**.
:::
...
<details class=details-quiz><summary>
What happened to the list on the right? _(Click to see the answer)_
</summary>
...
::: {.callout-tip appearance="simple"}
The VS Code Preview will show the image,
so it's an easy way to check that you got the path right.
:::
...
each **`<br>`** tag forces a linebreak:
