...
format:
  revealjs:
    ...
    include-after-body: ../slides-fit.html
...
- Briefly comment on basic (universally supported) vs. extended syntax.
--->
...
- The path is interpreted **relative to the Markdown file** ---
  therefore, for example, an image in an `img` subfolder needs `![](img/plot.png)`.

::: {.callout-tip appearance="simple"}
A caption can be added using `![Optional caption](image.jpg)`,
but this syntax is not supported in all contexts.
For example, it does not work in the Markdown preview in VS Code.
:::
