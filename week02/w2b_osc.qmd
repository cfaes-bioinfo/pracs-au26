...
<!----
TODO for next year: add why there are three clusters and how they differ.
---->
...
- This is a brief overview before we get started with OSC,
  and you'll learn more later in the course.
...
- What nodes are and which two types exist
- The file system at OSC and how to navigate it
- How to use the "OSC OnDemand" web portal
...
- It mostly comes down to **scale** --- e.g., your analysis may:
  - Take a long time to run
  - Need large numbers of processors or a large amount of memory
  - Involve too much data to store on your computer

- Additionally, your analysis may require:
  - Linux-only software
  - Specialized hardware, such as advanced GPUs
...
- OSC has:
  ...
  - Excellent infrastructure to access these resources
...
![](/img/osc_cluster-diagram_crop1.png){fig-align="center" width="95%" fig-alt="A diagram showing the structure of a supercomputer center with three main components: file systems, login nodes, and compute nodes"}
...
![](/img/osc_cluster-diagram_crop2.png){fig-align="center" width="95%" fig-alt="A diagram showing the structure of a supercomputer center with three main components: file systems, login nodes, and compute nodes"}
...
Windows is the odd one out: it has multiple file system starting points and
...
<details class="details-quiz">
<summary>Is the `guest` folder inside the `home` folder, or vice versa?</summary>
...
Like a regular computer (left), OSC (right) has a single file structure:
...
- A project is usually tied to a lab, research project, or course.
...
- It does so at the level of a project (not individual users).
:::
...
:::: callout-quiz
::: callout-note
#### Where should each of these go, Project (`/fs/ess`), Scratch, or Home?
...
- `jane` has created a personal folder in the `PAS3493` project,
  with the folder name the same as her OSC username, and you will soon do the same.

<details class="details-quiz">
<summary>What will be the path to your soon-to-be created personal folder?</summary>

For example: `/fs/ess/PAS3493/people/jane`
...
::: {.callout-warning appearance="simple"}
On the left, the `PAS3493` box in the [Home]{style="background-color:#FBE3D6"}
branch is faded because:
...
The two main types of nodes are **login** vs. **compute** nodes.
