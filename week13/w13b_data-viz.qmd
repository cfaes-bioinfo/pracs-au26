...
the development of graphics. Learning this grammar will help you to
...
create a plot in `ggplot2`, you pass your data as the first argument, and then add layers of
...
- Basic syntax of the `ggplot()` function, including aesthetic mappings with `aes()`
- How to create scatter plots, bar graphs, and box plots with `geom_point()`, `geom_col()`, `geom_bar()`, and `geom_boxplot()`
- How to add titles and labels with `labs()`, and customize themes with `theme()`
- How to facet plots using `facet_wrap()` and `facet_grid()`
- How to export plots in different formats

## Set up and Data preparation

### Open a new Quarto document in RStudio

You should already have a `week13` directory from Monday's lecture.
Create a new Quarto document by clicking `File` \> `New File` \>
...
_tidyverse_ and _palmerpenguins_.
The [tidyverse](https://www.tidyverse.org/) is a larger collection of
packages including `ggplot2`. Some of the other useful packages in the
tidyverse for data manipulation and analysis include `dplyr` for data
manipulation (**we learnt on Monday**), `tidyr` for data tidying and more. By loading the
...
::: {.callout-note}
It is good practice to always install packages directly in the R console. Otherwise, every time you render 
...
The `data()` function loads a specified data set; without arguments, it lists the available data sets.
...
It consists of body measurement data of 3 species of penguins collected from 3 islands
...
can be created using the `ggplot2` package. Use the `help()` function or the `?` operator that you learned about in
week 12 to read about the `ggplot()` function.
...
not make sense right now but we will see a couple of examples by adding
...
- What do you think will happen if you keep `color = species` outside of `aes()`?
  There are actually three different ways to do this -- try each one and see what happens:

  a. `ggplot(aes(x = ..., y = ...), color = species)` -- still inside `ggplot()`, but outside `aes()`
  b. `geom_point(color = species)` -- inside `geom_point()`, outside `aes()`
  c. `geom_point(color = "red")` -- inside `geom_point()`, outside `aes()`, but with a fixed value instead of a variable

<details>

<summary>Click for the solution</summary>

a. Inside `ggplot()` but outside `aes()`, `color = species` is silently ignored,
   so all points are colored black (the default) and no legend is added:
...
b. Inside `geom_point()`, outside `aes()`, the same `color = species` instead
   produces an error:
...
c. But with a fixed value instead of a variable name, it works fine
...
**Why does (b) fail while (c) works, given that both are outside `aes()`?**
Anything outside `aes()` is treated as a **fixed setting** rather than a mapping,
so it must be given a literal value (like `"red"`) that `geom_point()` can use
directly to color every point. `species` isn't a variable that exists outside
of the `penguins` data frame, so outside of `aes()`, R has no idea what it
refers to and throws an error.
In (a), by contrast, `ggplot()` simply ignores the `color` argument entirely
because it's outside `aes()` and not a recognized argument of `ggplot()`
itself, so `color = species` has no effect at all.
...
```{r}
## You can set `color = "red"` outside the `aes()` function, inside `geom_point()`.
penguins |> 
  ggplot(aes(x =  bill_length_mm, 
                     y = bill_depth_mm )) +
  geom_point(color = "red")
...
What if you have 10 treatments that you want to color? It requires a lot of typing. R has several functions
with a wide range of **color palettes**. Let's explore the `RColorBrewer` package, which has several color palettes.
To use the `RColorBrewer` package, first install it and then load it.

![](../img/color_palette.png){width="50%" fig-align="center"}

Let's pick the `Pastel1` color palette for our scatter plot, which needs to be used with the function `scale_color_brewer()`.
...
Let's compute the mean first manually to plot the data. On Monday, we learnt to group variable/s by the `group_by` function. Use the same concept here to calculate the mean body mass of the penguins for each island.
...
This time, let's save the plot as an object, `box_plot_body_mass_and_island`,
so we can reuse it later:
```{r}
box_plot_body_mass_and_island <- penguins |>
  ggplot(aes(x =  island,
             y = body_mass_g)) +
  geom_boxplot()

box_plot_body_mass_and_island
```
...
There are many other `geom`s available in `ggplot2` to construct bar graphs, histograms and more. 
[ggplot2 cheat
sheet](https://rstudio.github.io/cheatsheets/data-visualization.pdf) from RStudio provides an overview of the
...
problem is to facet the plot. Faceting allows you to create multiple panels based on the values of a 
...
- `facet_wrap()`: Use this function to facet by a single categorical variable. It allows to lay out
...
use the `Export` button in the `Plots` tab of the output pane. You can choose the file format, 
