...
can be created using the `ggplot2` package. Use the `help(?)` we learnt in
`week12` to read about `ggplot()` function.
...
Let's compute the mean first manually to plot the data. On Monday, we learnt to group variable/s by the `group_by` function. Use the same concept here to calculate the body mass of the penguins for each island.
