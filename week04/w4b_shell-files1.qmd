...
(The dates and project ID will differ in your output; mine is `PAS0471` ---
...
Everything you do for the rest of this lecture and for the next one happens
inside this one `sandbox` dir, so let's take a moment to look at the layout:
...
Run the command and see what it did.
...
- Different directory, same file name:
  ...
  ```bash
  # (You don't need the trailing slash in testdir1/, but it can be clearer.)
  cp newfile3.txt testdir1/
  ```

- Different directory, different file name:

  ```bash
  cp newfile3.txt testdir1/newfile3_backup.txt
  ```
...
```bash
# Equivalent to: cp testdir1/newfile3_backup.txt newfile3_backup.txt
cp testdir1/newfile3_backup.txt .
```
...
```bash
cp testdir1 testdir1_copy
```
```bash-out
cp: -r not specified; omitting directory 'testdir1'
```

The `-r` option is needed for recursive copying:

```bash
cp -r testdir1 testdir1_copy
ls testdir1_copy
```
```bash-out
newfile3_backup.txt  newfile3.txt
```
...
```
```{=html}
<pre class="bash-out"><code><span style="color: #0328fc;">butterflies</span>  newfile1.txt  newfile2.txt  newfile3_backup.txt  newfile3_copy.txt  newfile3.txt<br><span style="color: #0328fc;">testdir1</span>  <span style="color: #0328fc;">testdir1_copy</span>  <span style="color: #0328fc;">testdir2</span>  <span style="color: #0328fc;">testdir3</span></code></pre>
```
...
That is because both a file's name (e.g. `monarch.txt`) and the dir it is inside
(`butterflies/`) are part of its path (`butterflies/monarch.txt`).
...
- Same directory, different file name ("renaming"):
  ...
  ```bash
  mv newfile3_copy.txt newfile4.txt
  ```

- Different directory, same file name ("moving"):

  ```bash
  mv newfile4.txt testdir2/
  ```

- Different directory, different file name ("moving + renaming"):

  ```bash
  mv newfile3_backup.txt testdir2/newfile3_backup_old.txt
  ```
...
::: exercise
#### {{< fa user-edit >}} Exercise 4: File matching
...
such as copy (`cp`), move (`mv`), and delete (`rm`).
For example, we can:
