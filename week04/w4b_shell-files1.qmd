...
- Select multiple files with globbing (**this lecture**)
...
using globbing (later in this lecture).
:::
...
Non-recursive defaults act as a kind of safety mechanism,
since they make it harder to accidentally operate on very large amounts of data.]:
...
As the error message above hints,
...
::: callout-tip
#### Both the `cp` and `mv` (next up) commands will by default:

- Be **silent** when they succeed
  (use the `-v` option for **verbose** to make them report what they do).
- **Overwrite** existing files without reporting this
  (use the `-i` option for **interactive** to make them ask before overwriting).
:::

<hr style="height:1pt; visibility:hidden;" />

::: exercise
#### {{< fa user-edit >}} Self-study exercise: Copying a dir a second time

What do you think happens if you run the exact same command, `cp -r jar/ jar_copy`,
a second time? Make a prediction, then run it and check the result with `ls jar_copy/`.

<details><summary> {{< fa check-circle >}} &nbsp; _Click to see the answer._</summary>

Recall that for files, if the destination is an existing dir,
the copy is placed _inside_ it --- and the same is true for dirs.
Because `jar_copy` now already exists,
the second command creates a copy of `jar` _inside_ `jar_copy`:
...
```
```{=html}
<pre class="bash-out"><code><span style="color: #0328fc;">jar</span>  kiwi_backup.txt  kiwi.txt</code></pre>
```
</details>
:::
...
While copying a file creates a new copy of it, moving a file **does not**
(think: copy-paste versus cut-paste).

Also, **moving** a file versus **renaming** it is fundamentally the same operation:
in both cases, you are **changing its path**.
After all, both a file's name (e.g. `mango.txt`) and the dir that it's inside
...
- Same directory, different file name ("**renaming**"):
  ...
  ```bash
  mv -v kiwi_copy.txt plum.txt
  ```
  ```{=html}
  <pre class="bash-out"><code>renamed 'kiwi_copy.txt' -> 'plum.txt'</code></pre>
  ```
  ```bash
  ls
  ```
  ```{=html}
  <pre class="bash-out"><code>apple.txt  <span style="color: #0328fc;">box</span>  <span style="color: #0328fc;">cup</span>  fig.txt  <span style="color: #0328fc;">jar</span>  <span style="color: #0328fc;">jar_copy</span>  kiwi_backup.txt  kiwi.txt  plum.txt<br><span style="color: #0328fc;">pot</span></code></pre>
  ```

- Different directory, same file name ("**moving**"):

  ```bash
  # [Don't run this one: example only]
  mv -v plum.txt cup/
  ```
  ```{=html}
  <pre class="bash-out"><code>renamed 'plum.txt' -> 'cup/plum.txt'</code></pre>
  ```
...
- Different directory, different file name ("**moving + renaming**"):
  ...
  ```bash
  # [Don't run this one: example only]
  mv -v kiwi_backup.txt cup/kiwi_old.txt
  ```
  ```{=html}
  <pre class="bash-out"><code>renamed 'kiwi_backup.txt' -> 'cup/kiwi_old.txt'</code></pre>
  ```

Final notes:

- While we used `-v` above to make `mv` report what it does,
  this is **not required** for the command to work.
- Unlike `cp`, the `mv` command does not need an `-r` option to move or rename dirs!

::: {.callout-warning}
#### Avoid accidents due to typos by using Tab completion
Watch out for typos in a destination dir name: `mv plum.txt cups`
would not give an error even though there is no `cups` dir:
it would _rename_ `plum.txt` to a **file** called `cups`.
Including a trailing slash (`cups/`) protects against this,
because `mv` will then give an error if that dir doesn't exist
(the same goes for `cp`).

Avoiding these mistakes is much easier if you consistently use Tab completion
to type paths to existing dirs.
:::
...
As with `cp`, the **`-r`** option is needed to make the command work recursively:
...
::: callout-warning
#### There is no trash bin when deleting files in the shell, so use `rm` with caution!

`rm -r` can be very dangerous.
For example, `rm -r /` would try to remove the entire contents of the computer,
...
You'll often see `rm -rf` online: the `-f` (force) option makes `rm`
ignore nonexistent files and never ask for confirmation,
so it's even less forgiving. A couple of ways to take precautions:
...
Tab completion would have prevented this:
it automatically "escapes" the space with a backslash,
completing the name to `extra\ data`.
...
Globbing is a very useful shell technique to select multiple files at once,
using **wildcards** --- **characters with a special meaning**.
...
::: {.callout-warning}
#### Not able to paste?
Your browser may be blocking the paste action, but you can change that.
Find a small **clipboard icon** in the address bar of your browser, click it,
and allow pasting.
:::

Let's check the contents of the `sandbox` dir again:

```bash
ls
```
```{=html}
<pre class="bash-out"><code><span style="color: #0328fc;">cup</span>  fig.txt  <span style="color: #0328fc;">jar</span>  <span style="color: #0328fc;">jar_copy</span>  kiwi_backup.txt  kiwi.txt  plum.txt  <span style="color: #0328fc;">pot</span><br>sample1_R1.fastq  sample1_R2.fastq  sample2_R1.fastq  sample2_R2.fastq<br>sample3_R1.fastq  sample3_R2.fastq</code></pre>
```

We have a real mess now! Globbing will help us to select files to operate on without
having to type out all of their names.
...
For example, you can match only the FASTQ files with a `*` using:

```bash
# This matches files that end with "fastq"
ls *fastq
```
```bash-out
sample1_R1.fastq  sample2_R1.fastq  sample3_R1.fastq
sample1_R2.fastq  sample2_R2.fastq  sample3_R2.fastq
```

Or only the sample1 FASTQ files:
...
```bash
# This matches files whose names contain "R1" anywhere:
ls *R1*
```
```bash-out
sample1_R1.fastq  sample2_R1.fastq  sample3_R1.fastq
...
...the shell will perform globbing before it runs the `ls` command.
As such, the `ls` command will never see the `*` wildcard^[
If nothing matches, the shell leaves the pattern as is ---
which is why the error message for `ls *R1` above showed `*R1` literally.].
Instead, the command that is actually run is:

```bash
ls sample2_R1.fastq sample2_R2.fastq
```

You can see this expansion for yourself with `echo`,
which simply prints whatever arguments it receives:
...
So far, we've only used globbing with the `ls` command to list files.
But because the expansion is done by the shell,
globbing works with **any command**,
and is especially useful with commands that accept multiple paths as arguments,
such as copy (`cp`), move (`mv`), and remove (`rm`).
As some initial examples of how useful globbing is, we can:

- Remove all stray `.txt` files at once.
  Before removing files with a pattern,
  it's a good habit to first check what the pattern matches with `ls`:

  ```bash
  ls *.txt
  ```
  ```bash-out
  fig.txt  kiwi_backup.txt  kiwi.txt  plum.txt  sample1.txt  sample2.txt
  ```

  Then remove them
  (adding the `-v` option just to make `rm` report what it is removing):
...
Globbing is incredibly handy for working with large numbers of files,
...
::: exercise
#### {{< fa user-edit >}} Self-study exercise: Globbing pitfalls

a. Say your dir contained only two R1 FASTQ files, `sample1_R1.fastq` and `sample2_R1.fastq`.
   You want to copy them into another dir, but forget to type the destination:
   ...
   Knowing that the shell expands the pattern before `cp` runs, what would happen?

   <details><summary> {{< fa check-circle >}} &nbsp; _Click to see the answer._</summary>

   After the shell has expanded the pattern, `cp` receives two arguments:

   ```bash
   cp sample1_R1.fastq sample2_R1.fastq
   ```

   So, `cp` treats the second file as the destination,
   and it will silently overwrite `sample2_R1.fastq` with a copy of `sample1_R1.fastq`! 😳

   (With three or more matching files, you would instead get an error such as
   `cp: target 'sample3_R1.fastq' is not a directory`,
   because when `cp` gets more than two arguments, the last one has to be a dir.)
   </details>

b. You want to remove all `.txt` files in your working dir,
   but accidentally type a space after the `*`:
   ...
   A single stray space makes a big difference: `rm *.txt` removes all `.txt` files,
   but `rm * .txt` removes **all files** in your working dir
   (dirs are only spared because you didn't use `-r`,
   though `rm` prints an error for each of them),
   and it also complains that there is no file called `.txt`.

   A good habit is to first run `ls` with the same pattern to check what it matches,
   and only then run `rm`.
   </details>

c. What would have happened if you had used `*fastq*` instead of `*fastq`
   in the `mv` example above, i.e. `mv -v *fastq* fastq_files/`?

   <details><summary> {{< fa check-circle >}} &nbsp; _Click to see the answer._</summary>

   The pattern `*fastq*` also matches the `fastq_files` dir itself!
   So `mv` tries to move that dir into itself, which fails with an error.
   The FASTQ files are still moved:

   ```bash-out
   mv: cannot move 'fastq_files' to a subdirectory of itself, 'fastq_files/fastq_files'
   renamed 'sample1_R1.fastq' -> 'fastq_files/sample1_R1.fastq'
   [...]
   ```
   </details>
