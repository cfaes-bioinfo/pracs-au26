...
- ***With practice, using the shell is faster and more powerful than a GUI file***
  ***browser*** (certainly when working at OSC).
- You may run into situations where a GUI file browser is not available.
...
First, create a dir for this week
(we'll talk about the `mkdir` command later today):
...
2. Save the file inside `/fs/ess/PAS3493/people/$USER/week04`,
   e.g. as `lectureB.md`

_TODO: Do this in the file explorer instead._

## Intermezzo: VS Code revisited

#### Install two VS Code extensions

In the Activity Bar, click the `Extensions` icon. _TODO - ADD ICON IMAGE_

In the Extensions pane that now makes up the Primary (wide) Side Bar,
...
- **shellcheck** (by *timonwong*) --- this will check your shell scripts later on!
...
### Additional VS Code tips and tricks _TODO - add an activity around this_
...
- **Toggling (hiding/showing) the side bars**\
  ...
  - Or use keyboard shortcuts:
    ...
    - <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>B</kbd> for the activity/narrow side bar *TODO: Check this shortcut*
...
- **Auto Save**\
  To change the auto-saving behavior of documents in the editor,
...
- Entries in **[blue]{style="color: #0328fc"}** are directories
  (`fastq` and `ref` above)
- Entries in **black** (or white in dark themes) are regular files (`README.md` and `meta.tsv` above)
...
to list the contents of dirs other than your working dir,
specify one or more dirs as arguments:
...
2. Create a new file `monarch.txt` inside a new dir `butterflies`.
...
- Different directory, same file name ("moving"):
  ...
  ```bash
  # (You don't need the trailing slash to meta/, but this can be clearer.)
...
here, remove one of the files you created above:
