...
-   Understand the three components of Quarto: **YAML header, code chunks and text**

## Quarto

Quarto is an extension of Markdown that allows you to combine prose, code,
...
Create a new folder for `week14` by clicking the `New Folder` icon in the `Files` pane.
...
This will create a new unnamed `Untitled.qmd` file. Let's rename it, by
going to `File`, `Save As` and then save it as `quarto_intro.qmd` inside the `week14` directory.
...
executes code in the document and produces a plain Markdown file, which is
...
is any code that produces an error, your document will not be rendered.**
...
Additionally, it consists of a series of metadata such as title, author,
date etc. You can think of the YAML header as the settings of the Quarto
...
an entirely new theme through [**Sass**](https://quarto.org/docs/output-formats/html-themes-more.html).
...
::: callout-tip
-   Project level YAML: YAML settings can be specified at the project
...
-   Document level YAML: YAML settings can be specified at the document
    level at the top of each Quarto document. These are used to
...
A key with a set of ordered values can be specified in YAML sequences in
two different ways:
...
2.  Block syntax: Each item on a new line, preceded by a hyphen (`-`).
    **We saw an example of block syntax in the figure above**

![](../img/inline_syntax_quarto.png){width="50%" fig-align="center"}

In the above figure, we can see that `html:`, `theme: cosmo` 
and `toc: true` are nested under the `format:` key. Nesting in Quarto denotes the hierarchical relationship
...
a string contains special characters (e.g. #, :, etc). In general, I prefer to use 
...
4.  View the `diamonds` dataset using the `View()` function. Check the number of rows and columns in the
    `diamonds` dataset. Then select the first 5 columns, keep only rows with a carat greater than 0.23,
...
6.  Make your HTML document self-contained, render it, download and check the updated HTML file.

<details>

<summary>Click for the solution</summary>

_(Also add `embed-resources: true` to your YAML header.)_
