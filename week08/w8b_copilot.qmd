...
We will use a local installation of VS Code rather than the familiar VS Code Server
...
at OSC more generally -- while it required [some setup](../UA/UA4.qmd),
this is a more convenient way of using VS Code!

## Connecting VS Code to OSC

After doing [last week's ungraded assignment](../UA/UA4.qmd),
...
2. In the "Welcome" tab, you should already see your OSC host
   (e.g. `<user>@pitzer.osc.edu`) and the appropriate folder (`/fs/ess/PAS3493/people/<user>`)
   under "Recent".
   ...
   - If not: click "Connect to..." a little further up in the same tab
     ...
     move VS Code to your personal dir in the course's project dir
   ...
   <hr style="height:1pt; visibility:hidden;" />
   ![](../img/vs-code-connect-screenshot.png){fig-align="center" width="60%" fig-alt="A screenshot of the VS Code Welcome tab, showing the OSC host and course folder listed under Recent." .lightbox}
   </details>
...
Google Gemini also has a VS Code extension with similar functionality,
...
- You only have a limited number of suggestions per month^[
  Currently 2,000 inline code completions, plus a monthly allowance of
  "GitHub AI Credits" that also powers chat and agent requests.]
...
![_From the [VS Code docs](https://code.visualstudio.com/docs/copilot/setup)_](../img/setup-copilot-status-bar.png){fig-align="center" width="80%" fig-alt="A screenshot of the Copilot icon in the VS Code status bar, with the 'Set up Copilot' option shown." .lightbox}
...
![_From the [VS Code docs](https://code.visualstudio.com/docs/copilot/setup)_](../img/setup-copilot-sign-in.png){fig-align="center" width="80%" fig-alt="A screenshot of the VS Code dialog prompting you to continue with GitHub to sign up for Copilot Free." .lightbox}
...
![_The GitHub Copilot extensions you should install_](../img/copilot-extensions.png){fig-align="center" width="40%" fig-alt="A screenshot of the GitHub Copilot and GitHub Copilot Chat extensions listed in the VS Code Extensions sidebar." .lightbox}

4. Click on the Copilot icon in the bottom right again,
   and make sure to check all boxes at the bottom, e.g.:

![_Make sure to check all the boxes at the bottom.<br>The 'Markdown' box is shown in this case because we have a Markdown file open._](../img/copilot-checkboxes.png){fig-align="center" width="35%" fig-alt="A screenshot of the Copilot status bar menu with all file-type checkboxes, including Markdown, checked." .lightbox}
...
```bash
# You should be in /fs/ess/PAS3493/people/$USER
mkdir week08
cd week08
touch copilot.md
```
...
But instead of letting GitHub Copilot go off on its own to randomly suggest
ways to interrogate this file,
...
does it seem to have given a correct suggestion?
...
# Print a frequency table of all feature types in the GTF file
...
![](../img/copilot-inline-1.png){fig-align="center" width="80%" fig-alt="A screenshot of an inline Copilot suggestion in ghost text that does not correctly answer the prompt." .lightbox}

So I pressed the <kbd>⌘</kbd>+<kbd>I</kbd> and reported the failure:

![](../img/copilot-inline-2.png){fig-align="center" width="80%" fig-alt="A screenshot of the Copilot inline chat box, with a message reporting that the previous suggestion was incorrect." .lightbox}

Copilot then came up with a fix, which I could accept:

![](../img/copilot-inline-3.png){fig-align="center" width="80%" fig-alt="A screenshot of Copilot's inline chat proposing a fixed code suggestion." .lightbox}

Running the modified line did give the correct answer:

![](../img/copilot-inline-4.png){fig-align="center" width="80%" fig-alt="A screenshot of the terminal, showing the corrected code now producing the correct output." .lightbox}
:::
...
```bash
# List the top 5 largest files in the /fs/ess/PAS3493 directory and its subdirs

# Find all Markdown files in /fs/ess/PAS3493 and its subdirs

# Print a list of files that contain the word "cathemerium" in /fs/ess/PAS3493 and its subdirs
```
...
![](../img/copilot-hover-suggestions.png){fig-align="center" width="90%" fig-alt="A screenshot showing alternative Copilot suggestions revealed by hovering over the ghost text, with navigation arrows in the top right." .lightbox}
:::
...
<kbd>Ctrl</kbd>+<kbd>⌘</kbd>+<kbd>I</kbd> (Mac) /
...
![](../img/copilot-open-chat.png){fig-align="center" width="75%" fig-alt="A screenshot of the VS Code top bar, with the icon to open the GitHub Copilot Chat sidebar highlighted on the far left." .lightbox}

The chat sidebar should open on the right side of your VS Code window,
and look something like this:

![](../img/copilot-chat-bar.png){fig-align="center" width="65%" fig-alt="A screenshot of the GitHub Copilot Chat sidebar open on the right side of VS Code, showing copilot.md listed as context at the top." .lightbox}
...
The dropdown menu in the bottom-left corner of the chat window allows you to
select different **chat modes:**
...
#### About models
One feature of GitHub Copilot is that it allows you to choose between
completely different LLMs (GPT, Claude, Gemini, and others).
On the free plan (which is what we're using), however, Copilot picks a model
for you automatically for each request -- there's no model dropdown to choose from.
Paid Copilot plans do let you manually pick a specific model.
:::
...
![](../img/copilot-chat-script-explanation.png){fig-align="center" width="55%" fig-alt="A screenshot of Copilot Chat's response explaining the generated FastQC Bash script." .lightbox}

To create a new file with the suggested code,
click the `...` icon and then select "Insert into New File":

![](../img/copilot-chat-1-insert.png){fig-align="center" width="85%" fig-alt="A screenshot of the Copilot Chat '...' menu with the 'Insert into New File' option highlighted." .lightbox}
...
Let's save that as `fastqc.sh` within the `week08` dir.
...
![](../img/copilot-chat-key-changes.png){fig-align="center" width="70%" fig-alt="A screenshot of Copilot Chat's response listing the key changes after adding strict Bash settings to the script." .lightbox}
...
![](../img/copilot-chat-2-diff.png){fig-align="center" width="70%" fig-alt="A screenshot of the 'Apply in Editor' diff view, showing the proposed script changes to accept or reject." .lightbox}
...
# Check if an argument was provided
if [ $# -eq 0 ]; then
    echo "Error: No FASTQ file provided" >&2
    echo "Usage: $0 <fastq_file>" >&2
    exit 1
fi
...
![](../img/copilot-chat-edit-context.png){fig-align="center" width="60%" fig-alt="A screenshot of the Copilot Chat sidebar in Edit mode, with fastqc.sh selected as context." .lightbox}
...
![](../img/copilot-chat-edit-mode.png){fig-align="center" width="90%" fig-alt="A screenshot of Edit mode directly suggesting code changes in the editor to print the date before and after running FastQC." .lightbox}
...
![](../img/copilot-agent-fastqc.png){fig-align="center" width="90%" fig-alt="A screenshot of Copilot Agent mode finding the existing fastqc.sh script and proposing an example command to run it." .lightbox}
...
![](../img/copilot-agent-trimgalore2.png){fig-align="center" width="98%" fig-alt="A screenshot of Copilot Agent mode creating a new TrimGalore script based on existing scripts it found in the project dir." .lightbox}

It found TrimGalore scripts elsewhere in the project dir
(i.e. the open folder, which is `/fs/ess/PAS3493/people/$USER`),
...
![](../img/copilot-agent-featurecounts-full.png){fig-align="center" width="98%" fig-alt="A screenshot of Copilot Agent mode creating a new script to run FeatureCounts on BAM files." .lightbox}
...
![](../img/gemini-extension-install.png){fig-align="center" width="40%" fig-alt="A screenshot of the Gemini Code Assist extension listed in the VS Code Extensions sidebar." .lightbox}

2. Click the struck-through "AI star" icon in the thin bar at the very bottom to
   open the Gemini sidebar:

![](../img/gemini-star.png){fig-align="center" width="40%" fig-alt="A screenshot of the Gemini 'AI star' icon in the thin bar at the bottom of VS Code." .lightbox}

3. Click "Sign in":

![](../img/gemini-signin.png){fig-align="center" width="40%" fig-alt="A screenshot of the Gemini Code Assist sidebar with a 'Sign in' button." .lightbox}

4. Click "Open" to allow VS Code to open the external website
...
![](../img/gemini-auth.png){fig-align="center" width="40%" fig-alt="A screenshot in the browser confirming that Gemini authentication with a personal Google account succeeded." .lightbox}
...
![](../img/gemini-chat.png){fig-align="center" width="36%" fig-alt="A screenshot of the Gemini Code Assist chat window open in VS Code." .lightbox}
...
- "Slash commands" with `/`:
  ...
  - `/fix` to receive a proposed fix for the problems in the selected code
