...
- When adding **`-y`**, Conda will not ask you for confirmation to install.
...
Using a Conda environment works much like using a module:
you first make its programs available, and then run them as usual.
But while the term "load" is used for modules,
the term and command `activate` is used for Conda environments ---
it means the same thing.

You should be able to **activate** the TrimGalore environment you created above
...
- You avoid filling up your Home directory: Conda environments consist of many
  small files, and Home has a limit of 1 million files (as well as 500 GB of space).
